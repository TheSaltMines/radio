...
console.log('setting volume to '+level);  
    ...
    if (typeof msg == 'object') {
      ...
      if (arguments.length==1) {
        if (window.fluid) {
          ...
          $('#wrapper').append($("<div></div>").addClass('notification').html(msg).animate({
            'top': '20px'
  ...
  $(document).ready(function() {
    ...
    $('#intro').live({
      mouseover: function() {
        $('#intro #quickVolumeTweaks').show();
      },
      mouseout: function() {
        $('#intro #quickVolumeTweaks').hide();
      }
    });
    
    $('#quickVolumeTweaks .volume.button').live({
      click: function() {
        console.log('v='+$(this).attr('vol')+'&duration='+$(this).attr('duration'));
          $.ajax({
            url: '/data.php',
            dataType: 'json',
            data: 'v='+$(this).attr('vol')+'&duration='+$(this).attr('duration'),
            async: false,
            success: function(d) {
            }
          });
